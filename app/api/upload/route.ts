...
export async function POST(req: Request) {
  try {
    ...
    if (!file.type.startsWith('image/')) {
      return NextResponse.json({ error: 'Invalid file type', details: 'File must be an image' }, { status: 400 })
    }
    ...
    const normalizedFolderPath = 'avatars/'
    ...
    return NextResponse.json(
      {
        error: 'avatar upload request failed',
